...
class QueueManager {
  constructor() {
    this.queues = {};
    // Usar la misma conexión Redis para todo
  ...
  // Esperar a que Redis esté disponible antes de continuar
  async waitForRedis(retries = 30, interval = 2000) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        console.log(`⏳ Esperando a Redis... intento ${i + 1}/${retries}`);
        await new Promise(resolve => setTimeout(resolve, interval));
      }
    }
    throw new Error("No se pudo conectar a Redis después de varios intentos");
  }
